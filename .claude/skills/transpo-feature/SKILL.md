...
8. **Tests** — unitaire sur la règle métier (back), rendu + interaction sur l'UI ; multi-tenant : vérifier l'isolation (une requête d'un tenant ne voit pas les données d'un autre).
9. **Vérifier en exécutant** — dérouler le flux réel (pas seulement les tests) : basculer FR↔AR et clair↔sombre, cliquer chaque contrôle.
...
- [ ] Tests verts + flux vérifié à la main.
