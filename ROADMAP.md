...
- [~] **T4 — Console web (shell complet)** 🚧 *validé Docker* — refonte fidèle à la maquette : **shell** (sidebar groupée Opérations/Flotte/Facturation/Administration + topbar recherche/thème/avatar), thème Radix indigo/slate + bascule clair/sombre. **21 écrans** branchés à l'API : Dashboard, Commandes (recherche/filtres/pagination), Analytics, Fraude, Tournées, Dispatch, PC flotte, Zones, Hub, Retours, Véhicules, Chauffeurs, Tarification, Factures, Caisse, Reversement, Utilisateurs, Notifications, Avis, Modèles, Paramètres. Console servie en **build de prod** (déterministe). **E2E Playwright 5/5** (dont smoke des 21 écrans). Restent : assistant de création 3 étapes, détail commande (timeline+onglets), écrans en écriture (actions). — PRD-01
